...
import re
#import logging
#logging.basicConfig(filename='/var/ossec/logs/custom-opencti.log', encoding='utf-8', level=logging.DEBUG)
...
opencti_base_url = "http://<host>/graphql"
# OpenCTI Server API AUTH KEY
opencti_api_auth_key = "Bearer <api>"
# API - HTTP Headers
...
if event_source == 'windows':
    if event_type == 'sysmon_event1':
        ...
        sys.exit()
    api_json_body={"query": "\nquery StixCyberObservables($types: [String], $filters: [StixCyberObservablesFiltering], $search: String, $first: Int, $after: >    try:
    ...
    if event_type == 'sysmon_event3' and alert["data"]["eventdata"]["destinationIsIpv6"] == 'false':
        try:
            ...
            if ipaddress.ip_address(dst_ip).is_global:
                wazuh_event_param = dst_ip
                api_json_body={"query": "\nquery StixCyberObservables($types: [String], $filters: [StixCyberObservablesFiltering], $search: String, $first: I>
                try:
                    opencti_api_response = requests.post(opencti_base_url, headers=opencti_apicall_headers,json=api_json_body)
                except ConnectionError:
                    alert_output["opencti"] = {}
    ...
    if event_type == 'traefik':
        try:
            dst_ip = alert["data"]["srcip"]
            if ipaddress.ip_address(dst_ip).is_global:
                wazuh_event_param = dst_ip
                api_json_body={"query": "\nquery StixCyberObservables($types: [String], $filters: [StixCyberObservablesFiltering], $search: String, $first: I>
                try:
