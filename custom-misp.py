...
misp_base_url = "https://misp.cloud96.com/attributes/restSearch/"
# MISP Server API AUTH KEY
misp_api_auth_key = "2uIwrLNOFH0qIR6Jw1n0utmpJoeINnXo8Vh1yk8Q"
# API - HTTP Headers
...
if event_source == 'windows':
    ...
    if event_type == 'traefik':
        ...
        sys.exit()
elif event_source == 'windows':
    if event_type == 'sysmon_event_22':
        try:
            dst_ip = alert["data"]["win"]["eventdata"]["queryResult"]
            if ipaddress.ip_address(dst_ip).is_global:
                wazuh_event_param = dst_ip
                misp_search_value = "value:"f"{wazuh_event_param}"
                misp_search_url = ''.join([misp_base_url, misp_search_value])
